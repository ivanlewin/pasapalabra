...
def obtener_configuracion() -> tipos.configuracion:
    dicc_configuracion: dict = crear_diccionario_configuracion()
...
def ejecutar_partida(diccionario_como_lista: tipos.diccionario_como_lista, jugadores: List[tipos.jugador], cantidad_de_palabras_por_letra: dict[str, int], configuracion: tipos.configuracion) -> List[tipos.puntaje]:
    ...
    * Lewin, Iván
    ...
    for i in range(len(rosco)):
        ...
        if resultado == 'e':
            turno_del_jugador_actual = (
                turno_del_jugador_actual + 1) % len(jugadores)

    puntajes_de_la_partida = calcular_puntaje_de_la_partida(
        jugadas, jugadores, configuracion['PUNTAJE_ACIERTO']["valor"], configuracion['PUNTAJE_DESACIERTO']["valor"])
...
def ejecutar_juego() -> None:
    ...
    * Lewin, Iván
    ...
    diccionario_filtrado = filtrar_diccionario(
        diccionario_crudo, configuracion['LONGITUD_PALABRA_MINIMA']["valor"])
    ...
    mostrar_total_de_palabras(
        cantidad_de_palabras_por_letra, configuracion['CANTIDAD_LETRAS_ROSCO']["valor"])
    ...
    while continuar_jugando and jugadas_restantes_disponibles > 0:
        puntaje_de_la_partida = ejecutar_partida(
            diccionario_como_lista, jugadores, cantidad_de_palabras_por_letra, configuracion)
        sumar_puntajes(puntaje_de_la_partida, puntajes_del_juego)
        mostrar_puntajes_parciales(
            puntaje_de_la_partida, puntajes_del_juego, jugadas_restantes_disponibles)
        jugadas_restantes_disponibles -= 1
        if jugadas_restantes_disponibles > 0:
            print()
            continuar_jugando = preguntar_seguir_jugando(
                jugadas_restantes_disponibles)
            if (continuar_jugando is False):
                print()

    mostrar_resumen_juego(
        configuracion['MAXIMO_PARTIDAS']["valor"] - jugadas_restantes_disponibles, puntajes_del_juego)
